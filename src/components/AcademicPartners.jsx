...
export default function AcademicPartners() {
  ...
  return (
    <section className="relative py-8 border-y border-emerald-900/10 bg-white/70 backdrop-blur-md overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
          {/* Label / Badge */}
          <div className="flex-shrink-0 flex items-center gap-2.5 md:border-r md:border-slate-200 md:pr-8">
            <div className="w-8 h-8 rounded-lg bg-emerald-100 border border-emerald-300 flex items-center justify-center text-[#15736B]">
          ...
          <div className="relative w-full overflow-hidden py-1">
            <InfiniteSlider speed={35} speedOnHover={15} gap={36}>
              {partners.map((partner, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-3 px-4 py-2 rounded-xl bg-white border border-slate-200/90 shadow-sm hover:border-[#4EA840] transition-colors group cursor-default"
                >
                  <div className="w-2 h-2 rounded-full bg-[#4EA840] group-hover:scale-125 transition-all" />
                  ...
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-emerald-50 text-[#15736B] border border-emerald-200 whitespace-nowrap font-semibold">
                    {partner.tag}
                  </span>
                </div>
              ))}
